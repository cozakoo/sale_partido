...
**¿Por qué Node.js 18+?**
- Necesario para ejecutar scripts de sincronización de GitHub (octokit, dotenv)
- Requiere npm para instalar dependencias

**Si no tienes Node.js:**
- Descarga desde: https://nodejs.org/ (LTS - versión recomendada)
...
📁 Archivo: doc/.local/data/HUS.json
```
...
## 📌 Formas Recomendadas de Ejecutar Scripts

### Opción 1: Usando `./scripts/run.sh` (RECOMENDADO)

Entrada centralizada para todos los comandos:

```bash
# GitHub Projects
./scripts/run.sh github:sync          # Sincronizar HUs
./scripts/run.sh github:validate E4-H08  # Validar HU
./scripts/run.sh github:board         # Descargar tablero

# Git workflow
./scripts/run.sh git:branch E4-H08 descripcion  # Crear rama
...
Si prefieres ejecución directa:

```bash
# Node scripts (GitHub Projects)
node scripts/github/sync-github-projects.js
node scripts/github/download-board.js

# Bash scripts
./scripts/git/create-branch.sh E4-H08 descripcion
./scripts/setup/init.sh
```

**Recomendación:** Usa `./scripts/run.sh` para consistencia y mensajes formatados.
...
2. **Descarga el tablero actual:**
   ...
   # Ver en: doc/.local/data/BOARD.md
   ```
...
4. **Crea una rama:**
   ...
   # Automáticamente sincroniza GitHub Projects
   ```
...
6. **Haz commit:**
   ...
   # Sin validación automática - commits libres
   ```
